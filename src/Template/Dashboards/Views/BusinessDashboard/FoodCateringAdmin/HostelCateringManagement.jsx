...
const initialWeeklyMenu = {
  'Monday': { Breakfast: [{ name: 'Idli', description: 'Steamed rice cakes', status: true }], Lunch: [{ name: 'Rice & Dal', description: 'Simple lunch', status: true }], Dinner: [], Snacks: [] },
...
const summaryCounters = [
  { label: 'Total Dishes', icon: <RestaurantIcon />, color: 'primary.main' },
  { label: 'Unique Items', icon: <FastfoodIcon />, color: 'secondary.main' },
  { label: 'Active', icon: <LunchDiningIcon />, color: 'success.main' },
  { label: 'Inactive', icon: <DinnerDiningIcon />, color: 'error.main' },
];

export default function HostelCateringManagement() {
  const theme = useTheme();
  const [selectedWeek, setSelectedWeek] = useState(weekOptions[0]);
  const [thisWeekMenu, setThisWeekMenu] = useState(initialWeeklyMenu);
  const [nextWeekMenu, setNextWeekMenu] = useState(initialWeeklyMenu);
  ...
  const [batchMenu, setBatchMenu] = useState(() => weekDays.reduce((acc, day) => {
  ...
  // Analytics
  const allItems = Object.values(thisWeekMenu).flatMap(day => mealTypes.flatMap(meal => day[meal]));
  const totalDishes = allItems.length;
  const uniqueItems = new Set(allItems.map(i => i.name)).size;
  const activeItems = allItems.filter(i => i.status).length;
  const inactiveItems = totalDishes - activeItems;
  const summaryValues = [totalDishes, uniqueItems, activeItems, inactiveItems];
  ...
  const updateCurrentWeekMenu = (updater) => {
    if (selectedWeek === 'This Week') {
      setThisWeekMenu(updater);
    } else {
      setNextWeekMenu(updater);
    }
  };
  const handleDialogSave = () => {
    updateCurrentWeekMenu(prev => {
      const updated = { ...prev };
      const items = [...updated[editCell.day][editCell.meal]];
      if (editCell.item) {
        // Edit
      ...
      updated[editCell.day][editCell.meal] = items;
      return updated;
    });
    setDialogOpen(false);
  ...
  const handleDelete = (day, meal, item) => {
    updateCurrentWeekMenu(prev => {
      const updated = { ...prev };
      updated[day][meal] = updated[day][meal].filter(i => i !== item);
      return updated;
    });
  ...
  const handleClearWeek = () => {
    if (selectedWeek === 'This Week') {
      setThisWeekMenu(initialWeeklyMenu);
    } else {
      setNextWeekMenu(initialWeeklyMenu);
    }
  ...
  // Handler for opening info dialog
  ...
  // Handler for opening batch add dialog
    ...
    updateCurrentWeekMenu(prev => {
      ...
      weekDays.forEach(day => {
        mealTypes.forEach(meal => {
          updated[day][meal] = batchMenu[day][meal].filter(name => name.trim()).map(name => ({ name, description: '', status: true }));
        });
      });
      return updated;
    });
            ...
            {mealTypes.map(meal => (
              <Grid container key={meal} alignItems="center" sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
                ...
                {weekDays.map(day => (
                  <Grid item xs key={day} sx={{ p: 1 }}>
                    <Box display="flex" flexDirection="column" gap={1}>
                      {(thisWeekMenu[day][meal] || []).map((item, idx) => (
                ...
                <Grid item xs={1} sx={{ p: 1 }}>
                  <IconButton color="info" onClick={() => handleInfoOpen(weekDays, meal, weekDays.map(day => thisWeekMenu[day][meal]))}>
                    <InfoIcon />
                  </IconButton>
                </Grid>
              </Grid>
